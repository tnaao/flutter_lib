...
class AgoraRtmPlugin : FlutterPlugin, MethodCallHandler {
    ...
    companion object {
        val clients = HashMap<Long, RTMClient>()
    ...
    private fun handleClientMethod(methodName: String?, params: Map<String, Any>, result: Result) {
        ...
        when (methodName) {
            ...
            "xRelease" -> {
                RtmEventHandler.getInstance(applicationContext).sendEvent(RtmEvent.Release())
                runMainThread {
            ...
            "destroy" -> {
                ...
                clients[clientIndex]?.client?.release();
                ...
                client.login(
                    ...
                    object : ResultCallback<Void> {
                        ...
                        override fun onFailure(code: ErrorInfo) {
                            Log.e(
                                "AgoraRtmPlugin",
                                "login failed: ${code.errorCode},${code.errorDescription}"
                            )
                            runMainThread {
                                result.success(
                                    hashMapOf(
                                        "errorCode" to code.errorCode,
                                        "errorDescription" to code.errorDescription
                                    )
                                )
                            }
                        }
                    ...
                    object : ResultCallback<Void> {
                        ...
                        override fun onFailure(code: ErrorInfo) {
                            runMainThread {
                                result.success(
                                    hashMapOf(
                                        "errorCode" to code.getErrorCode(),
                                        "errorDescription" to code.getErrorDescription()
                                    )
                                )
                            }
                        }
            ...
            "subscribePeersOnlineStatus" -> {
                val peerIds: Set<String>? = (args?.get("peerIds") as ArrayList<String>).toSet()
                client?.subscribePeersOnlineStatus(peerIds, object : ResultCallback<Void> {
