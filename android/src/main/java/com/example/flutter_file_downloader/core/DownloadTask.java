...
public class DownloadTask {
    ...
    public DownloadTask(Activity activity, String url, String name, String savePath, DownloadCallbacks callbacks) {
        this.callbacks = callbacks;
        this.activity = activity;
        this.url = url;
        this.name = name;
    ...
    public void startDownloading() {
        ...
        String saveLocal = Environment
                .getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
                .getAbsolutePath() + "/" + getDownloadedFileName();

        if (callbacks != null) {
            if (savePath != null && savePath.length() > 0) {
                FileUtil.INSTANCE.downloadFile(this.url, savePath, callbacks);
            } else {
                FileUtil.INSTANCE.downloadFile(this.url, saveLocal, callbacks);
            }
            return;
        }
