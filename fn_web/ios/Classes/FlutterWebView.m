...
[_webView evaluateJavaScript:imgJS completionHandler:^(id _Nullable imgUrl, NSError * _Nullable error) {
    if (imgUrl) {
        ...
        highlightedButtonTitle:nil
         otherButtonTitles:@[@"Save Image", @"Open QR Code"]];
        } else {
        actionSheet = [[FSActionSheet alloc] initWithTitle:nil
        delegate:self
...
- (void)FSActionSheet:(FSActionSheet *)actionSheet selectedIndex:(NSInteger)selectedIndex{
    switch (selectedIndex) {
        ...
        case 1:
        {
            ...
            // 内部应用打开
            [_webView loadRequest:[NSURLRequest requestWithURL:qrUrl]];
        }
            break;
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo{
    ...
    [_webView makeToast: message duration:2.0
...
- (UIView*)view {
//    UIView* rootView =  [[UIView alloc] initWithFrame:_frame];
