...
typedef StarCuIPullIconBuilder = Widget Function(
    BuildContext context, IndicatorState state, double animation);

/// Text widget builder.
typedef StarCuITextBuilder = Widget Function(
    BuildContext context, IndicatorState state, String text);

/// Message widget builder.
typedef StarCuIMessageBuilder = Widget Function(
...
class StarCustomIndicator extends StatefulWidget {
  ...
  final StarCuIPullIconBuilder? pullIconBuilder;

  /// Text style.
  final TextStyle? textStyle;

  /// Build text.
  final StarCuITextBuilder? textBuilder;

  /// Text style.
  final TextStyle? messageStyle;

  /// Build message.
  final StarCuIMessageBuilder? messageBuilder;
  ...
  @override
  State<StarCustomIndicator> createState() => _StarCustomIndicatorState();
}

class _StarCustomIndicatorState extends State<StarCustomIndicator>
    with TickerProviderStateMixin<StarCustomIndicator> {
  ...
  @override
  void didUpdateWidget(StarCustomIndicator oldWidget) {
        ...
        [
          ...
          if (widget.showText)
            VStack(
              [
                _buildText().box.make(),
                _buildMessage().xVisible(widget.showMessage),
              ],
            ).box.margin(EdgeInsets.only(left: widget.spacing)).make(),
        ],
      ),
    );
  }
